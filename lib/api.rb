class Api
  ...
  def create_shows
    @show_list.each do |show_hash|
      ...
      show_instance = Show.new(name: show_info['name'].nil? ? "Unlisted" : show_info['name'], genre: show_info['genres'].nil? ? "Unlisted" : show_info['genres'], type: show_info['type'].nil? ? "Unlisted" : show_info['type'])
